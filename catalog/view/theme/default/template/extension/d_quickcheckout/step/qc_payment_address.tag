<qc_payment_address>
    <div class="step qc-payment-address">
        ...
        <div if={getConfig().payment_address.display == 1 && getState().config.guest.payment_address.style == 'clear'}>
            <h4 class="uk-hidden">{  getLanguage().payment_address.heading_title }</h4>
            <p if={getLanguage().payment_address.text_description}>{  getLanguage().payment_address.text_description } </p>
            ...
            <div class="{ (getAccount() != 'logged' 
                || (getAccount() == 'logged' 
                    && (getSession().payment_address.address_id == '0' 
                    || !getSession().payment_address.address_id))) ? '' : 've-hidden' }">
                <form id="payment_address_fields" class="payment-address-fields saForm uk-margin-remove">
                    <div class="uk-child-width-1-1 uk-child-width-1-2@m uk-grid-medium" data-uk-grid>
                    <div
                        each={ field_id in fields}
                        if={ (getConfig().payment_address.fields[field_id])}
                        class="qc-field ve-fiel {field_id} { (field_id == 'address_1') ? 'uk-width-1-1' : '' } { (field_id == 'address_2' || field_id == 'company') ? 'uk-width-1-2 uk-width-1-4@m' : '' } { (field_id == 'customer_group_id' || field_id == 'country_id' || field_id == 'telephone' || field_id == 'fax' || field_id == 'newsletter' || field_id == 'heading' || field_id == 'agree' || field_id == 'shipping_address') ? 'uk-hidden' : '' }"
                        sort_order={ getConfig().payment_address.fields[field_id].sort_order }
                        field_id={field_id}
                        step="payment_address"
                        field={ getConfig().payment_address.fields[field_id] }
