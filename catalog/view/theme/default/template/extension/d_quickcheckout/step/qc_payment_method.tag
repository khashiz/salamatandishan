<qc_payment_method>
    <div class="step  qc-payment-method">
        ...
        <div if={getConfig().payment_method.display == 1 && getState().config.guest.payment_method.style == 'clear'}>
            <h3 class="uk-text-bold uk-text-yellow uk-h4 font">{ getLanguage().payment_method.heading_title }</h3>
            <p if={getLanguage().payment_method.text_description}>{  getLanguage().payment_method.text_description } </p>

            <div each={error, error_id in getError().payment_method} if={error} class="ve-alert ve-alert--danger has-error"><qc_raw content="{error}"></qc_raw></div>
            <form class="uk-margin-remove uk-text-zero" id="payment_method_list" if={getConfig().payment_method.display_options == 1 && getSession().payment_methods}>
            <div class="uk-grid-small uk-child-width-1-1" data-uk-grid>

                <div if={getState().config.guest.payment_method.input_style == 'radio'}  each={ payment_method, name in getSession().payment_methods }>
                    <label for="{ payment_method.code }" if={ payment_method} class="uk-flex uk-flex-middle checkoutLabel uk-button uk-button-default uk-padding-small uk-border-rounded uk-lineheight-normal uk-text-right { getSession().payment_method.code == payment_method.code ? 'selected' : '' }">
                        <img if={ getConfig().payment_method.display_images == 1} class="uk-hidden" src="{payment_method.image}" />
                        <input
                            type="radio"
                            name="payment_method"
                            value="{ payment_method.code }"
                            ...
                            <div class="uk-display-inline-block">
                                <span class="uk-text-secondary font fnum uk-text-small">{ payment_method.title }</span>
                                <span class="uk-text-muted font fnum uk-text-tiny">{ payment_method.text }</span>
                            </div>
                    </label>
                </div>
