<qc_shipping_method>
    <div class="step qc-shipping-method">
        ...
        <div if={ getConfig().shipping_method.display == 1 && getState().config.guest.shipping_method.style == 'clear' }>
            <h3 class="uk-text-bold uk-text-yellow uk-h4 font">{ getLanguage().shipping_method.heading_title }</h3>
            <p if={getLanguage().shipping_method.text_description}>{  getLanguage().shipping_method.text_description } </p>
            ...
            <form class="uk-margin-remove uk-text-zero" id="shipping_method_list" if={getState().config.guest.shipping_method.display_options == 1 && getSession().shipping_methods}>
                ...
                class="" >
                    <div if={shipping_method}>
                        ...
                        <div each={ quote, index in shipping_method.quote } >
                            <label for="{ quote.code }" class="uk-flex uk-flex-middle checkoutLabel uk-button uk-button-default uk-padding-small uk-border-rounded uk-lineheight-normal uk-text-right {getSession().shipping_method.code == quote.code ? 'selected' : ''}">
                                <input
                                type="radio"
                                ...
                                <div class="uk-display-inline-block">
                                    <span class="uk-text-secondary font fnum uk-text-small">{ quote.title }</span><br>
                                    <span class="uk-text-muted font fnum uk-text-tiny">{ quote.text }</span>
                                </div>
                            </label>
                        </div>
                    </div>
