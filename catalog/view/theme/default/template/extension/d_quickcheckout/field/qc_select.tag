<qc_field_select>
    ...
    <div if={ isVisible() }  class="field-sortable d-vis ve-clearfix { (opts.error && isRequired()) ? 've-field--error' : ''}">
        <label class="uk-form-label uk-margin-small-bottom uk-text-secondary uk-display-block uk-text-bold font" for="{ opts.step }_{ opts.field.id }">
        ...
        <div>
            <select
                ...
                ref="input"
                class="uk-select uk-border-rounded uk-width-1-1 uk-text-right uk-text-secondary font ve-inpu d-vis qc-select{ (getState().config.guest[opts.step].fields[opts.field_id].search == 1) ? 'selectpicker' : ''} { isRequired() ? 'qc-required' : 'qc-not-required'} { opts.field.id }"
                required="{ isRequired() }"
