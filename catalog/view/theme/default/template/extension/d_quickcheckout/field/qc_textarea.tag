<qc_field_textarea>
    ...
    <div if={ isVisible() } class="uk-margin-medium-top field-sortable d-vis  ve-clearfix  { (opts.error && isRequired()) ? 've-field--error' : ''}">
        <label class="uk-form-label uk-margin-small-bottom uk-text-secondary uk-display-block uk-text-bold font" for="{ opts.step }_{ opts.field_id }">
        ...
        <div>
            <textarea
                ...
                class="uk-textarea uk-border-rounded uk-width-1-1 uk-text-right uk-text-secondary font ve-inpu d-vis validate { (opts.field.require) ? 'qc-required' : 'qc-not-required'} { opts.field.type } { opts.field.id }"
