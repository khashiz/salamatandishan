<qc_field_text>
    ...
    <div if={ isVisible() } class="field-sortable d-vis ve-clearfix  { (opts.error && isRequired()) ? 've-field--error' : ''}">
        <label class="uk-form-label uk-margin-small-bottom uk-text-secondary uk-display-block uk-text-bold font" for="{ opts.step }_{ opts.field_id }" if={getLanguage()[opts.step][opts.field.text]}>
        ...
        <div>
            <input
                ...
                class="uk-input uk-border-rounded uk-width-1-1 uk-text-right uk-text-secondary font ve-inpu d-vis validate { isRequired() ? 'qc-required' : 'qc-not-required'} { opts.field.type } { opts.field_id }"
